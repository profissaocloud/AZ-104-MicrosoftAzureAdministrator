...
1. From the Cloud Shell pane, run the following to create the virtual machines. Type each command line separately and execute them separately:
...
1. On the **Security Settings** blade, note that **Enable soft delete for cloud workloads** is **Enabled**.
...
1. Within the Remote Desktop session to the **az104-10-vm1** Azure virtual machine, start an Edge web browser, browse to the [Azure portal](https://portal.azure.com), and sign in using your labuserxxxxx@clouders.com.br credentials.
...
1. On the **Security Settings** blade, Disable **Enable soft delete for cloud workloads** and also disable **Enable soft delete and security settings for hybrid workloads** and click **Save**.
