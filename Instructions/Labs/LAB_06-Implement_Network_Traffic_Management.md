...
1. From the Cloud Shell pane, run the following to create the three virtual networks and four Azure VMs into them by using the template and parameter files you uploaded:
   ...
   ```
    ...
    > 1. Check the location in which the 'xxxxx-az104-06-rg1' resource group is deployed. You can run `az group show -n xxxxx-az104-06-rg1 --query location` in your CloudShell to get it.
...
1. Add a peering with the following settings (leave others with their default values) and click **Add**:
    ...
    | Virtual network: Allow access to remote virtual network | **selected (default)** |
    | Virtual network: Allow traffic to remote virtual network | **selected** |
    | Virtual network: Allow traffic forwarded from the remote virtual network (allow gateway transit) | **selected** |
    | Virtual network: Use Virtual network gateway or route server| **unselected (default)** |
...
1. On the **Network Watcher** blade, check that NetworkWatcher_eastus is present in list, this represent the region **(US) East US** you are using. 
...
1. On the **Network Watcher - Connection troubleshoot** blade, initiate a check with the following settings (leave others with their default values):
    ...
    > **Note**: It may take a few minutes for the resource group to be listed. If you don't want to wait, try this: delete the Network Watcher, create a new Network Watcher, and then retry Connection Troubleshoot. Change the **xxxxxx** to your labuser ID.

    | Setting | Value |
    | --- | --- |
    | Subscription | the name of the Azure subscription you are using in this lab |
    | Resource group | **xxxxx-az104-06-rg1** |
